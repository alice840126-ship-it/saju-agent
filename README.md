...
├── prompts/사주.md             # ← LLM(Claude 등)용 13단계 풀이 프롬프트
...
**풀이 엔진**은 Claude/ChatGPT 같은 LLM이다. `prompts/사주.md`는 13단계로
구성된 '대만신' 페르소나 프롬프트로, 결정론 출력값을 입력받아 자연어 풀이를
생성한다. (Claude Code에서는 `~/.claude/commands/사주.md`로 슬래시 커맨드화.)
...
입력으로 전달하면 13단계 풀이가 나온다 (총평 → 핵심성정 → 진로·재물 → 인간관계 →
혼인·자녀 → 건강 → 대운 흐름 → 권고).
